...
class ActionProcessor:
    ...
    def process_im_action(self, action):
        ...
        if SymElementsParser().get_form_values(action)['action'].startswith('docs'):
        ...
        elif SymElementsParser().get_form_values(action)['action'] == 'buy':

            cusip = SymElementsParser().get_form_values(action)['cusip']
            ...
            for i in sales_room_members:
                self.bot_client.get_stream_client().add_member_to_room(stream, i['id'])
